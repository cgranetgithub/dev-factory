"""
Tests for the QA summary that is fed back to the developer on a retry.
...
seen from inside the QA container.
"""

from __future__ import annotations

from devfactory.qa.runner import CONTAINER_WORKDIR, QARunner
...
def test_summary_strips_the_container_mount_from_ruff_paths():
    ...
    runner = QARunner(image="test-image")
    ruff = {
        "issues": [
...
def test_summary_strips_the_container_mount_from_every_tool():
    ...
    runner = QARunner(image="test-image")
    mypy = {"errors": [f"{CONTAINER_WORKDIR}/devfactory/qa/runner.py:12: error: bad type"]}
    pytest = {
        "passed": 0,
        "failed": 1,
    ...
    assert CONTAINER_WORKDIR not in summary
    assert "devfactory/qa/runner.py:12" in summary
